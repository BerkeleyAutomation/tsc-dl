function [ figOutput ] = plotState( x_curr, target_curr, fAx, background)
   if nargin < 2
...
%     hRobot.MarkerHandle.FaceColorData = uint8(255*[0;0;1;0.5]);
    theta = x_curr(3);
    r = 1;
    hRobot_heading = plot (fAx,[x_curr(1), x_curr(1)+r*cos(theta)], ...
        [x_curr(2), x_curr(2)+r*sin(theta)], ...
        'b-', 'LineWidth', 3);
    
    %%    
    figOutput = [hTarget, hRobot_body, hRobot_heading ]; %return figure handle
