...
if __name__ == "__main__":
	...
	argparser.add_argument("mode", help = "python tscdl.py Z _test_ --visual_feature 5_PCA.p Choose from 1) W (Kinematics only), 2) Z (Visual features only), 3) ZW (Kinematics+Vision) 4) plot")
	...
	if args.mode == "ZW":
		...
		if not args.visual_feature:
			print "ERROR: Please specify visual feature as: --visual_feature <feature file>.p"
			os.remove(constants.PATH_TO_CLUSTERING_RESULTS + args.output_fname + ".txt")
			sys.exit()
		...
		print "ERROR: Specify correct mode"
